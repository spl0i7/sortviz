...
pub struct ExchangeSort<T> where T: Ord + Clone + Debug {
    elements: Vec<T>,
    pos_i: usize,
    pos_j: usize,
}

impl<T> ExchangeSort<T>  where T: Ord + Clone + Debug  {
...
impl<T> StepSortingAlgorithm<T> for ExchangeSort<T> where T: Ord + Debug + Clone{}

impl<T> Iterator for ExchangeSort<T>
